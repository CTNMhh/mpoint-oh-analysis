  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-50 to-white py-16 pt-30 px-4">
          ...
          >
            <div className="mb-4">
              <h2 className="text-2xl font-bold mb-2 flex items-center gap-2">
                {plan.name}
                {plan.highlight && <Star className="w-5 h-5 text-yellow-400" />}
              </h2>
              <div className="flex items-end justify-center mb-2">
                <span className="text-4xl font-extrabold text-gray-900">{plan.price}</span>
                {plan.period && <span className="text-gray-500 ml-1 mb-1">/ {plan.period}</span>}
              </div>
              <p className="text-gray-500 mb-4">{plan.features[0]}</p>
            </div>
            <ul className="mb-6 space-y-2 flex-1">
              {plan.features.map((feature, i) => (
                <li key={feature + i} className="flex items-center gap-2 text-gray-800">
              ...
              {plan.premium.length > 0 && (
                <div className="mt-2 space-y-2">
                  {plan.premium.map((feature, i) => (
                    <li key={feature + i} className="flex items-center gap-2 text-[rgb(228,25,31)] font-medium">
            ...
            <Link
              ...
              className={`block w-full py-3 rounded-lg font-semibold text-center transition-all duration-200 ${
                plan.highlight
                  ? "bg-[rgb(228,25,31)] text-white hover:bg-red-700"
      ...
      <div className="max-w-3xl mx-auto mt-12 text-center text-gray-500 text-sm">
        ...
        <Link href="/contact" className="text-[rgb(228,25,31)] underline hover:text-red-700">
